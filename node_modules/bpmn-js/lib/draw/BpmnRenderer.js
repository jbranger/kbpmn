...
function BpmnRenderer(eventBus, styles, pathMap, priority) {
  ...
  //Letter
    'bpmn:MessageEventDefinition': function(p, element, isThrowing) {
      ...
      if (element.type.indexOf('StartEvent')  != -1){
    	  var stroke = '#39B54A';
    	  var fill = isThrowing ? '#39B54A' : 'none';
    }

      else if (element.type.indexOf('Intermediate')!= -1 || element.type.indexOf('Boundary') != -1 ) {
    	  var stroke = '#F7BD3C';
    	  var fill = 'none';
    	  //var fill = isThrowing ? 'blue' : 'none';   
    	  if (isThrowing){
    		  var fill = '#F7BD3C';
    		  var stroke = 'white';
    	  }
    	  ...
    	  var stroke = '#ED1C24';
    	  var fill = 'none';
    	//  var fill = isThrowing ? 'orange' : 'none';   
    	  if (isThrowing){
    		  var fill = '#ED1C24';
    		  var stroke = 'white';
    	  }
...
//Ticks
    ...
    'bpmn:EscalationEventDefinition': function(p, event, isThrowing) {
      ...
      else if (isIntermediateEvent(event)) {
      var stroke = '#F7BD3C';
      var fill = isThrowing ? '#F7BD3C' : 'none';     
    }
      else {
    	  var stroke = '#ED1C24';
    	  var fill = isThrowing ? '#ED1C24' : 'none';   	  
      }
    ...
    //TODO Link Event Definition
      if (isStartEvent(event)){
    	  console.log(event.type + "This is event type of Triangle")
    	  var stroke = '#39B54A';
    	  var fill = isThrowing ? '#39B54A' : 'none';
    	  }
      else if (isIntermediateEvent(event)) {
    ...
    //  var fill = isThrowing ? 'brown' : 'none';
    ...
    'bpmn:IntermediateEvent': function(p, element) {
      var outer = renderer('bpmn:Event')(p, element, {stroke: '#F7BD3C', strokeWidth: 1 });
      /* inner */ drawCircle(p, element.width, element.height, INNER_OUTER_DIST, { stroke: '#F7BD3C', strokeWidth: 1, fill: 'none' });
    ...
    'bpmn:ReceiveTask' : function(p, element) {
      ...
      if (semantic.instantiate) {
        drawCircle(p, 28, 28, 20 * 0.22, { stroke: '#41B4E7', strokeWidth: 1 });
    ...
    'bpmn:DataObject': function(p, element) {
      ...
      var elementObject = drawPath(p, pathData, { stroke: '#41B4E7', fill: 'white' });
    ...
    'bpmn:BoundaryEvent': function(p, element) {
      ...
      /* inner path */ drawCircle(p, element.width, element.height, INNER_OUTER_DIST, assign(attrs, { stroke:stroke, fill: 'none' }));
...
function isStartEvent(event){
	...
	//console.log(event.type.indexOf('StartEvent') != -1);
	return(event.type.indexOf('StartEvent') != -1);
	//return (event.type === 'bpmn:StartEvent');
}
function isIntermediateEvent(event){
//	console.log(event.type);
	return (event.type.indexOf('Intermediate') != -1 || event.type.indexOf('BoundaryEvent') != -1);
	//return (event.type ==='bpmn:IntermediateThrowEvent' || event.type === 'bpmn:IntermediateCatchEvent');
}

function isEndEvent(event){
//	console.log(event.type);
	return (event.type.indexOf('EndEvent') != -1);
	//return (event.type === 'bpmn:EndEvent');
}
